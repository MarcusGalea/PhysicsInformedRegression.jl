...
function spline_derivatives(f::Vector, t::Vector)
    ...
    scaled_itp = [scale(interpolate(uvals[i,:], BSpline(Linear())),xs) for i in 1:size(uvals,1)]
    du_splines = [[first(gradient(scaled_itp[i], tval)) for i =1:length(scaled_itp)] for tval in t]
    return du_splines
end
